...
$(function () {
    ...
    ****************************************************************************************************/
	...
	function fn_loadWord(){
		var param = {};
		Util.request("/mamager/inDataMng/mainBusinessMngInit.do", null, function(resultData){
	...
	function fn_clickBusiData(getName){
		...
		param.no = getName;
		console.log("!!!!!");
		Util.request("/mamager/inDataMng/mainBusinessMngClick.do", param, function(resultData){
			if( update ){
				//추진사업 상세 데이터 함수 호출
    ...
    ****************************************************************************************************/
	...
	function fn_searchBusi(){
		var param = {};
		param.busiName = $("#selectBusiName").val();
		Util.request("/mamager/inDataMng/mainBusinessMngSearch.do", param, function(resultData){
			//$("#targetMonth").val(resultData.date.date);
			//goalName = resultData.busiList;
			if( resultData.busiDescList.length > 0 ){
				//추진사업 테이블 리스트 함수 호출
	...
	function fn_busiSaveParam(){
		...
		var extensionList = dataName.split(".");
		var extension = "";
		if( extensionList.length > 0 ){
			
			for( var i = (extensionList.length-1) ; i < extensionList.length; i++ ){
	...
	function fn_busiSave(){
		...
		//확장자 체크 (한글, PDF, 엑셀)
		if( param.get('extension').toLowerCase() == '' || param.get('extension').toLowerCase() == 'pdf' || param.get('extension').toLowerCase() == 'hwp' || param.get('extension').toLowerCase() == 'hwpx' || param.get('extension').toLowerCase() == 'xls' || param.get('extension').toLowerCase() == 'xlsx'){
			...
			alert("PDF, EXCEL, 한글 파일만 올릴 수 있습니다.");
			return;
		}
	...
	function fn_updateData(param){
		// Ajax 방식으로 파일 업로드
		Util.requestFileUplaod("/mamager/inDataMng/mainBusinessUpdate.do", param, function(result){
	...
	function fn_saveInsertData(param){
		// Ajax 방식으로 파일 업로드
		Util.requestFileUplaod("/mamager/inDataMng/mainBusinessCreate.do", param, function(result){
        	if(result.resultValue == "Y"){
        		update = false;
	...
	function fn_fileDownload(no){
		var requestUrl = "/mamager/inDataMng/mainBusinessDwonload.do?no="+no;
	...
	function fn_busiDelete(){
		...
		Util.request("/mamager/inDataMng/mainBusinessMngDelete.do", param, function(resultData){
