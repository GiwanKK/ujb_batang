...
//			contentType : "application/json",				// 서버에 데이터를 보낼 때 사용 content - type
	...
	//의정부 연계 
	function fn_getRealCCTVList(){
		
		Util.request("/dashBoard/communication/cctvInfoList.do", null, function(resultData){
			fn_setStreamCCTVLayer(JSON.parse(resultData.cctvInfoList[0].jsonBuildObject.value))
			console.log("resultData : ", JSON.parse(resultData.cctvInfoList[0].jsonBuildObject.value))
		})
	}
	
	//지도에 stream cctv point 레이어 추가
	function fn_setStreamCCTVLayer(cctvPointData) {
		console.log("cctvPointData : ",cctvPointData)
		if(map.hasImage('stream_cctv')) {
			map.removeImage('stream_cctv');
		}
		if(map.getLayer('cctvStreamLayer')) {
			map.removeLayer('cctvStreamLayer');
		}
		if(map.getSource('cctvStreamLayer')) {
			map.removeSource('cctvStreamLayer');
		}
		/////////////////////////////////
		map.loadImage('/dist/images/communication/stream_cctv.png', function(error, image) { // ES5 equivalent
			  

			if (error) console.log("error : " , error);
			 
			// Add the image to the map style.
			map.addImage('stream_cctv', image);
			 
			map.addSource('cctvStreamLayer', {
				'type' : 'geojson',
				'data' : cctvPointData
			});
			map.addLayer({
				'id': 'cctvStreamLayer',
				'type': 'symbol',
				'source': 'cctvStreamLayer', 
				'layout': {
					'icon-image': 'stream_cctv',
					'icon-size': 1
					}
			});
			map.on('click', 'cctvStreamLayer', function(e) { // ES5 equivalent
				layerOpenClose(true);
				console.log("e.features[0] : ", e.features[0])
				var cctvUrl = e.features[0].properties['cctv_url'];
				var cctvName = e.features[0].properties['name'];
				fn_getCCTVStreamUrl(cctvUrl, cctvName);
				
				});
		});
	}
	
	function fn_getCCTVStreamUrl(cctvUrl, cctvName){
		$("#cctvVideo").attr("src", cctvUrl);
		$("#cctvName").text(cctvName);
		var hls = new Hls();
	    hls.loadSource(cctvUrl); // 동영상경로
