...
$(function(){
	...
	function fn_popData(){
		...
		Util.request(url, param, function(resultData){
			...
			//인구 현황
			if( resultData.popStatus.length > 0 || resultData.popStatusDt.length > 0  ) fn_popStatus(resultData.popStatus[0], resultData.popStatusDt[0]);
			//총인구별 추이
	...
	function fn_popStatus(data, dataDt){
		$("#popTotal").html('');
		$("#popGenderRate").html('');
	...
	function fn_popAgeStatusChart(data, sum){
		...
		popStatusData.dataFieldsValue1 = "atrb02";
		popStatusData.dataFieldsValue2 = "atrb03";
