...
$(function(){
	...
	function fn_localEconomyData(){
		...
		Util.request(url, {dateYm: $("#startDate").val()}, function(resultData){
			...
			fn_dateTxt(resultData.dateEc , resultData.dateSt , resultData.dateTy, resultData.dateLl)
			map.setLayoutProperty('vworld', 'visibility', 'none');
		});
	}
...
function fn_dateTxt(dateEc, dateSt, dateTy, dateLl){
	var dateEcString = "";
	var dateStString = "";
	var dateTyString = "";
	var dateLlString = "";
	if( dateEc ) {
		dateEcString += "( "+dateEc.maxdate.substr(0,4) + "년  ";
		dateEcString += parseInt(dateEc.maxdate.substr(4,2)) + "월 기준_반기별 )";
		$("#dateTxtEc").text(dateEcString);
	}
	if( dateSt ) {
		dateStString += "( "+ dateSt.maxdate.substr(0,4) + "년  ";
		dateStString += parseInt(dateSt.maxdate.substr(4,2)) + "월 기준_분기별 ) ";
		$("#dateTxtSt").text(dateStString);
	}
	if( dateTy ) {
		dateTyString += "( "+dateTy.maxdate.substr(0,4) + "년  ";
		dateTyString += parseInt(dateTy.maxdate.substr(4,2)) + "월 기준_분기별 )";
		$("#dateTxtTy").text(dateTyString);
	}
	if( dateLl ) {
		dateLlString += "( "+dateLl.maxdate.substr(0,4) + "년  ";
		dateLlString += parseInt(dateLl.maxdate.substr(4,2)) + "월 기준_분기별 )";
		$("#dateTxtLl").text(dateLlString);
	}
}