...
var CHART = {
		...
		dashBoard_pop_ageStatus :  function(displayDivTagID, configData, returnCallBack){
			...
			maleRange.label.fill = chart.colors.list[1];
			maleRange.label.dy = 20;
			maleRange.label.fontWeight = '600';
			maleRange.grid.strokeOpacity = 1;
			maleRange.grid.stroke = female.stroke;//수정 (왜 female.stroke지..)

			var femaleRange = valueAxis.axisRanges.create();
			femaleRange.value = 0;
			femaleRange.endValue = 25;
			femaleRange.label.text = "Female";
			femaleRange.label.fill = chart.colors.list[0];
			femaleRange.label.dy = 20;
			femaleRange.label.fontWeight = '600';
			femaleRange.grid.strokeOpacity = 1;
			femaleRange.grid.stroke = male.stroke;//수정
		...
		createBarChart: function(params) {
			...
			function createSeries(item) {
				...
				if( !params.reverse ) {
					...
					if( '{valueY}' != 0 ) bullet.label.text = '{valueY}';
					...
					series.columns.template.events.on("sizechanged", function (ev) {
						if (ev.target.dataItem && ev.target.dataItem.bullets) {
							...
							ev.target.dataItem.bullets.each(function(id, bullet) {
								if ( params.elementId == 'innovationChart') {
									if( ev.target.dataItem.valueY == 0 ) {
										bullet.label.isHiding == true;
										bullet.label.fill = am4core.color("#4440");
										console.log("!!! bullet : ", bullet.label)
										console.log("!!! bullet.text : ", bullet.label.text)
									}else{
					...
					//bullet.dx = -15;
					if( '{valueX}' != 0 )bullet.label.text = '{valueX}';
...
//				series.strokeOpacity = 0.5;
		...
		createPieBarChart: function(params) {
			...
			columnChart.numberFormatter.numberFormat = "##.##";
			// Create axes
