		...
		data.forEach(function(item,index){
			...
			if( item.sort == '1' ){
				...
				//감사담당관
					...
					//below_area d_only
					//htmlSort5 += '<dt class="b_yellow">비서실장</dt>';
					if(item.rnk == null || item.rnk == undefined || item.rnk == ''){
						htmlSort6 += '<dt class="b_yellow"  style ="cursor: pointer;">';
					}else{
						htmlSort6 += '<dt class="show-modal-btn"  style ="cursor: pointer;">';
					}
			...
			//동별
				...
				if(  item.deptOdr == '3' && item.gradeId == '14' || item.deptOdr == '3' && item.gradeId == '999999' ){
					if( cnt6 > 0 ) {
						htmlSort5 += '</div>';
						htmlSort5 += '</div>';
						htmlSort5 += '</div>';
