...
$(function () {
    ...
    ****************************************************************************************************/
	...
	function fn_loadWord(){
		...
		param.nowPageCnt = 10;
		param.nextNo = 0;
		Util.request("/mamager/inDataMng/dataStatusInit.do", param, function(resultData){
...
//				if( (index+pageLimit) == page.nowPage) {
	...
	function fn_searchSchma(){
		...
		param.nowPageCnt = 10;
		param.nextNo = 0;
		Util.request("/mamager/inDataMng/dataStatusSearch.do", param, function(resultData){
			if( resultData.scmaStatusList.length > 0 ){
				//스키마 검색 테이블 리스트 함수 호출
				fn_schmaTableList(resultData.scmaStatusList);
